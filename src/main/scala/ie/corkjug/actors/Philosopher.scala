...
import ie.corkjug.actors.Homework.{Subject, Philosophy}
...
object Philosopher {
  ...
  def props() = Props(classOf[Philosopher])
}
...
class Philosopher extends Actor with ActorLogging {
  ...
  private def considerExistence() = {
    ...
    if (lifeIsALottery == 1) {
      throw new MoralCrisis(s"Philosopher is experiencing a moral crisis while copying Kant")
    }
  }
}
